...
class CustomShape {
    ...
    if (inBody) {
      ...
      for (Vec2D v : poly.vertices) {
        ...
        if (distance < closestDistance) { 
          closestDistance = distance; closestPoint = v; 
        } 
  ...
  void display() { // get the pixel coordinates of the body 
    ...
    fill(col); // depending on the r this combi-code displays either a polygon or a circle 
    ...
    popMatrix(); 
  } // if the shape moves off-screen, destroy the box2d body (important!) // and return true (which will lead to the removal of this CustomShape object) 
  boolean done() { 
    Vec2 posScreen = box2d.getBodyPixelCoord(body); 
